...
export class EmployeeModel {
  ...
  static async updateProfilePicture(employeeId, profilePicture) {
    const result = await connection.query(
      "UPDATE EMPLOYEE SET picture = ? WHERE ID = ?",
      [profilePicture, employeeId]
    );
    return result;
  }
  ...
  static async getProfilePicture(employeeId) {
    const [profilePicture] = await connection.query(
      "SELECT picture FROM EMPLOYEE WHERE ID = ?",
      [employeeId]
    );
